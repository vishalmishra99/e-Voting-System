...
$result_4 = mysqli_query($conn,"SELECT * FROM voter WHERE field_1='$get_field_1' ");
...
else if (strlen($get_field_12) != 10) 
{
	$response["success"] = 3;
	echo json_encode($response);
}
else if(mysqli_num_rows($result_4))
{
	$response["success"] = 5;	
	echo json_encode($response);
}
...
{
	$result = mysqli_query($conn,"INSERT INTO voter( email, field_1, field_2, field_3, field_4,
